...
GLboolean printShaderInfoLog(GLuint shader, const char* error_str)
{
    // コンパイル結果を取得する
    GLint status;
    glGetShaderiv(shader, GL_COMPILE_STATUS, &status);
    if (status == GL_FALSE) std::cerr << "Compile Error in " << error_str << std::endl;
...
GLuint createParticleProgram(const char* vsrc, const char* fsrc)
{
    ...
    glBindAttribLocation(program, 2, "instance_size");
    glBindAttribLocation(program, 3, "instance_rotation");
    glBindFragDataLocation(program, 0, "fragment");
    glLinkProgram(program);
